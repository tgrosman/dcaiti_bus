...
# creates new file
def helperMethod(filter, filename, nonNumericQuoting):
    if(nonNumericQuoting):
        filter.to_csv(filteredGTFSDirName+filename, sep=",", index = False, quoting=csv.QUOTE_NONNUMERIC)
    else:
        filter.to_csv(filteredGTFSDirName+filename, sep=",", index = False, quoting=csv.QUOTE_NONE, escapechar="\\")
        removeEscapedChars(filteredGTFSDirName+filename)

# removes unnecessary data and invokes helperMethod to create new file
def helperMethod2(filter, filename, columnsToDrop, nanValueIncluded, nonNumericQuoting):
    if(nanValueIncluded):
        removeNaN = filter.replace('"nan"','""')
        removeUnnecessaryData = removeNaN.drop(columns=columnsToDrop)
        filteredData = removeUnnecessaryData.rename(columns=lambda x: '"' + str(x) + '"')
    else:
        filteredData = filter.drop(columns=columnsToDrop)
    helperMethod(filteredData, filename, nonNumericQuoting)
...
# converts values to String. Convertion of specialformat columns: if a value is empty, writes it as empty values, otherwise as String
def typeConverter(filter, specialformatCol, *stringColumns):
    for col in stringColumns:
        if(col == specialformatCol):
            filter[col] = filter[col].apply(lambda x: '"' + str(x) + '"' if(len(str(x))>3) else None)
        else: filter[col] = filter[col].apply(lambda x: '"' + str(x) + '"')
    return filter
...
helperMethod2(filterForAgency,'/agency.txt', ["agency_phone","agency_lang"], False, True)
...
helperMethod(removeSchienenersatzverkehr, '/routes.txt', True)
...
typeConverter(filterForTrips, "block_id", "route_id","trip_headsign","trip_short_name","block_id")
helperMethod2(filterForTrips, '/trips.txt', ["wheelchair_accessible", "bikes_allowed"], True, False)
...
typeConverter(dataStopTimes, "", "stop_id", "stop_headsign")
filterForStopTimes = dataStopTimes[dataStopTimes["trip_id"].isin(dataTripsNew["trip_id"])]
helperMethod2(filterForStopTimes, '/stop_times.txt', ["pickup_type", "drop_off_type"], True, False)
...
typeConverter(dataStops, "parent_station", "stop_id","stop_code","stop_name","stop_desc","stop_lat","stop_lon","parent_station","platform_code","zone_id")
filterForStops = dataStops[dataStops["stop_id"].isin(dataStopTimesNew["stop_id"])]
helperMethod2(filterForStops, '/stops.txt', ["level_id"], True, False)
...
helperMethod2(filterForTransfers, '/transfers.txt', [], True, False)
...
helperMethod2(filterForPathways,'/pathways.txt', ["stair_count", "max_slope", "length", "traversal_time", "signposted_as"], False, True)
...
helperMethod(filterForShapes, '/shapes.txt', True)
